...
const char* ssid = "Tang4";          //Tên mạng Wifi mà Socket server của bạn đang kết nối
const char* password = "hoichilinhnhe";  //Pass mạng wifi ahihi, anh em rãnh thì share pass cho mình với.
 
char* host = "dothigiangwebcontrol.herokuapp.com";  //Địa chỉ IP dịch vụ, hãy thay đổi nó theo địa chỉ IP Socket server của bạn.
...
void GetAduinoSend(){
  ...
  while (mySerial.available() > 0) {
    inByte = mySerial.readString();
  }
  if(inByte != ""){
  Serial.println("Arduino : ");
  Serial.print(inByte);
  Serial.println("");
  client.send("LED_STATUS",inByte);
  }
   mySerial.flush();
}
